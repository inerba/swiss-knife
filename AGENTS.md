...
- `src/tools/inspect-save/`: ispezione visiva di elementi con overlay (guide tratteggiate, padding/margin con pixel, radius), snapshot al click e scheda Info nel pannello. `entrypoints/inspect-save.ts` segue lo stesso ciclo di vita di contrasti/media-picker: iniezione su richiesta, port legato al `documentId`, comandi tastiera dal pannello, pulizia su Esc o navigazione. Prima del PNG nasconde overlay e gli elementi fixed/sticky o sovrapposti che non appartengono alla selezione, poi ritaglia sul canvas; per elementi più grandi del viewport riusa i tasselli di Screenshot.
