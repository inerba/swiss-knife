...
- `src/lib/floating-window.ts`: modalità di apertura (pannello laterale o finestra mobile), iniezione della finestra mobile e canale verso il suo host nella pagina. `entrypoints/floating-window.ts` crea al clic un host in top layer con shadow root chiuso e un iframe su `floating.html` (unica `web_accessible_resources`, `use_dynamic_url`); `entrypoints/floating/` è la stessa `App` in versione finestra. `entrypoints/background.ts` tenta sempre `sidePanel.open()` nel gesto e disattiva il pannello in modalità finestra mobile: vedi `docs/adr/0001` e `docs/adr/0002` prima di cambiarlo. Nella finestra mobile la scheda di lavoro è quella che la contiene (`activeTab()` usa `tabs.getCurrent()`); le catture passano da `captureVisibleTab` in `src/lib/browser.ts`, che nasconde la finestra; ogni sessione di selezione sulla pagina chiama `concealFloatingWindow()` all’avvio e la sua release alla fine (conferma, annullamento, errore, chiusura).
...
- `src/tools/inspect-save/`: ispezione visiva di elementi con overlay (guide tratteggiate, padding/margin con pixel, radius), click che fissa l'elemento (`locked` sul port), navigazione padre/figlio e conferma esplicita prima dello snapshot e della scheda Info nel pannello. `entrypoints/inspect-save.ts` segue lo stesso ciclo di vita di contrasti/media-picker: iniezione su richiesta, port legato al `documentId`, comandi tastiera dal pannello, pulizia su Esc o navigazione. Prima del PNG nasconde overlay e gli elementi fixed/sticky o sovrapposti che non appartengono alla selezione, poi ritaglia sul canvas; per elementi più grandi del viewport riusa i tasselli di Screenshot.
...
- `src/tools/browser-context/`: prompt per agente e report Markdown in inglese (markup pulito, regole CSS con origine e at-rule, stati, pseudo-elementi, ereditarietà, valori calcolati diversi dal default, variabili). Il prompt assembla un preambolo fisso, l’eventuale richiesta di modifica e un sottoinsieme predefinito dei fatti già raccolti; non sintetizza. `entrypoints/browser-context.ts` usa `runPickerHost` con `buildContextPayload`. Copia su richiesta (prompt o report completo) con ripiego manuale, download del report in `swiss-knife/browser-context/`.
...
- Al cambio scheda o navigazione, conserva lo strumento selezionato ma invalida risposte asincrone, risultati e selettori della pagina precedente. Nella finestra mobile il cambio scheda non invalida nulla (salta l’invalidazione su `tabs.onActivated` con `isFloatingWindow()`): la navigazione chiude la finestra. Non avviare mai scansioni o catture automaticamente: mostra il messaggio comune e attendi un nuovo gesto dell’utente. Fa eccezione l’apertura di uno strumento la cui unica azione è selezionare (Browser context): il clic sulla scheda dello strumento avvia già la selezione, mai dopo un cambio scheda.
