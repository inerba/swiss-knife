...
- `src/tools/inspect-save/`: ispezione visiva di elementi con overlay, snapshot al click e scheda Info nel pannello. `entrypoints/inspect-save.ts` segue lo stesso ciclo di vita di contrasti/media-picker: iniezione su richiesta, port legato al `documentId`, pulizia su Esc, spegnimento toggle o navigazione. Il PNG usa `captureVisibleTab` e il crop condiviso con Screenshot.
