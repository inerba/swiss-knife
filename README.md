...
Apri il pannello e scegli **Ispeziona e salva**. Il pulsante accanto al titolo attiva il puntamento in pagina: l'etichetta diventa **Clicca una sezione** e compare un overlay con evidenziazione, padding, tooltip essenziale e coordinate. Passa col mouse sulle sezioni, usa ↑ per ampliare al contenitore padre, ↓ per tornare al livello precedente, Invio o clic per confermare, Esc per annullare. Il clic non attiva link o pulsanti sottostanti.

Dopo la selezione il pulsante torna inattivo, l'overlay sparisce dalla pagina e il pannello mostra la scheda **Info** dell'elemento: tag, selettore, classi, tipografia, sfondo, bordo, spaziatura, effetti e layout. Un'anteprima PNG adattata al pannello, **Copia codice** (snippet HTML+CSS del sottoalbero sanitizzato), **Scarica immagine** (PNG) e **Scarica codice** (file HTML) completano lo strumento. Se l'elemento era più grande del viewport o parzialmente fuori schermo, l'anteprima PNG mostra solo la porzione visibile e compare un avviso.
