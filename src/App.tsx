...
import { ArrowLeft, PictureInPicture2, PocketKnife, Settings, SlidersHorizontal, X } from 'lucide-react';
...
import { connectFloatingWindow, isFloatingWindow, postToFloatingHost, showFloatingWindow } from './lib/floating-window';
export function App() {
  ...
  useEffect(() => {
    if (!floating) return;
    title.current?.focus();
    void connectFloatingWindow(setSelected).catch(error => console.error('Impossibile collegare la finestra', error));
  }, [floating]);
  async function detach() {
    setDetachStatus('');
    try {
      const tab = await activeTab();
      await showFloatingWindow(tab.id, 'open', selected);
      await browser.sidePanel.close({ windowId: tab.windowId });
    } catch (error) { setDetachStatus(explainError(error)); }
  }
  const settingsButton = <button className="icon-button" aria-label="Apri impostazioni" onClick={() => void browser.runtime.openOptionsPage()}><Settings /></button>;
  ...
  return <>
    {floating ? <header className="brand floating-bar"
      onPointerDown={event => { if ((event.target as Element).closest('button')) return; drag.current = { x: event.screenX, y: event.screenY }; event.currentTarget.setPointerCapture(event.pointerId); }}
      onPointerMove={event => { const last = drag.current; if (!last) return; drag.current = { x: event.screenX, y: event.screenY }; postToFloatingHost({ type: 'drag', dx: event.screenX - last.x, dy: event.screenY - last.y }); }}
      onPointerUp={() => { if (drag.current) postToFloatingHost({ type: 'drag-end' }); drag.current = null; }}
      onPointerCancel={() => { if (drag.current) postToFloatingHost({ type: 'drag-end' }); drag.current = null; }}>
      <span className="brand-icon" aria-hidden="true"><PocketKnife /></span><h1 ref={title} tabIndex={-1}>Swiss Knife</h1>{settingsButton}<button className="icon-button" aria-label="Chiudi" onClick={() => postToFloatingHost({ type: 'close' })}><X /></button>
    </header> : <header className="brand"><span className="brand-icon" aria-hidden="true"><PocketKnife /></span><div><h1>Swiss Knife</h1><p>Piccoli strumenti, a portata di clic.</p></div><button className="icon-button" aria-label="Stacca nella pagina" title="Stacca nella pagina" onClick={() => void detach()}><PictureInPicture2 /></button>{settingsButton}<span className="version">{browser.runtime.getManifest().version}</span></header>}
    <main>
      {detachStatus && <p role="alert" className="detach-status">{detachStatus}</p>}
      {!floating && <GlobalSiteAccessNotice />}
      {tool ? <>
        ...
        <p className="footnote">{floating ? 'Gli strumenti lavorano su questa pagina, quando lo chiedi tu.' : 'Gli strumenti lavorano sulla scheda attiva, quando lo chiedi tu.'}</p>
      </>}
    </main>
  </>;
}
