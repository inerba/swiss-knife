...
export async function startSession(tabId: number, signal: AbortSignal, onSelection: (result: MediaSelection) => void, onStatus: (status: string) => void): Promise<PickerSession> {
  ...
  let closed = false;
  // The session outlives the selection (file reads), so the window comes back as soon as the choice ends.
  ...
  function close() {
    ...
    pending.clear(); port.disconnect(); reveal();
  }
  ...
  port.onMessage.addListener(message => {
    ...
    else if (message.type === 'selection' && Array.isArray(message.result?.images)) { reveal(); onSelection(message.result); }
    else if (message.type === 'cancelled') { reveal(); onStatus('Selezione annullata. Premi Nuova selezione.'); }
    else if (message.type === 'error') { reveal(); onStatus(String(message.error)); }
