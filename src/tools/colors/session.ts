...
export async function startColorSession(tabId: number, signal: AbortSignal, onStatus: (status: string) => void, onResult: (result: PickedElement) => void, onEnd: (reason: 'cancelled' | 'error' | 'disconnected') => void = () => {}) {
  ...
  const timeout = setTimeout(() => {
    if (closed) return;
    close(); onStatus('Il selettore non risponde. Premi Da elemento per riprovare.'); onEnd('error');
  }, 10000);
  ...
  port.onMessage.addListener(message => {
    ...
    if (message.type === 'ready') { clearTimeout(timeout); onStatus('Punta un elemento e clicca. ↑ amplia, ↓ restringe, Esc annulla.'); }
