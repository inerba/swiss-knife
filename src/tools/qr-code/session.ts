...
export async function startQrPagePick(tabId: number, windowId: number, signal: AbortSignal, onStatus: (value: string) => void, onImage: (dataUrl: string) => void, onEnd: () => void) {
  ...
  let closed = false; const reveal = concealFloatingWindow(); const close = () => { if (closed) return; closed = true; signal.removeEventListener('abort', close); port.disconnect(); reveal(); };
